...
AT SELECTION-SCREEN ON VALUE-REQUEST FOR p_dir.
  cl_gui_frontend_services=>directory_browse( CHANGING selected_folder = p_dir ).
...
FORM class_download.
  ...
  SELECT seoclass~clsname INTO TABLE @DATA(lt_seoclass)
         FROM tadir INNER JOIN seoclass ON seoclass~clsname = tadir~obj_name
         WHERE tadir~object     =  'CLAS'
         AND   tadir~obj_name   IN @s_class
         AND   tadir~devclass   IN @s_pack
         AND   seoclass~clsname IN @s_class.
  LOOP AT lt_seoclass INTO DATA(ls_seoclass).
*  LOOP AT s_class.

    DATA: lv_class TYPE seoclskey.
    lv_class = ls_seoclass-clsname.  "s_class-low.
    CALL FUNCTION 'SEO_CLASS_TYPEINFO_GET'
      EXPORTING
        clskey        = lv_class  "class
...
*      EXPORTING
*        default_extension    = 'xml'
*        default_file_name    = CONV string( lv_class ) "mtd_name
*        file_filter          = cl_gui_frontend_services=>filetype_xml
*      CHANGING
*        filename             = file_name
*        path                 = p_dir  "path
...
*    CHECK user_action = cl_gui_frontend_services=>action_ok.
    ...
    localfile = p_dir && '/' && CONV char50( lv_class ) && '.xml'.  "full_path.
