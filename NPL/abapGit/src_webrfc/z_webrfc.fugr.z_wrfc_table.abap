...
*"----------------------------------------------------------------------
  ...
  " http://127.0.0.1:8000/sap/bc/webrfc?_FUNCTION=Z_WRFC_TABLE&callback=jsonCallback&method=Z_WRFC_GET_DUMPS

  TRY.
      DATA(lo_table) = NEW zcl_wrfc_table( query_string[] ).
*      return_code = lo_interface->call( ).
*      html[] = lo_interface->get_html( ).
      DATA(gv_jsonp) = lo_table->get_json( ).
...
*  ENDIF.
  ...
  SELECT SINGLE tabname INTO @DATA(lv_tabname) FROM dd02l WHERE tabname  = @iv_sql_table
                                                          AND   as4local = 'A'.
  IF ( sy-subrc = 0 ).
...
*    ASSIGN ls_work TO <ls_data> CASTING TYPE (iv_sql_table).
      ...
      DATA: lo_exception TYPE REF TO cx_root,
            lv_msg       TYPE zwrfc_ty_s_messages-msg.
      TRY.
...
*                                       IV_ROWS
    ...
    ENDIF.
  ELSE.
    PERFORM add_message USING    'E' 'Tabella di estrazione inesistente'
                        CHANGING lt_messages[].
    PERFORM create_jsonp_2 USING    iv_callback
                                    lt_messages[]
                           CHANGING html[].
